...
<div class="wrapper">
  ...
  <h3>Choose a field to Edit and click the save button</h3> 
      ...
      <form method="post" action="transactionEdit.php" name="registration" class="form-horizontal">
...
<option>customer_phone</option>
</select>
</div>
</div>

<div class="form-group">
<label class="col-sm-2 control-label"> TransactionId </label>
<div class="col-sm-8">
<input type="number" name="transactionId" id="transactionId"  class="form-control"  required="required">
...
if(isset($_POST['save'])){
          require_once'database.php';
            ...
            if($_POST['dep']=="productName"){             
       
                $field="productName";                 
            }
      
             else if($_POST['dep']=="Amount"){
                $field="Amount";
            }
             else if($_POST['dep']=="type"){
                $field="type";
            }
             else if($_POST['dep']=="quantity"){
                $field="quantity";
                }
                else if($_POST['dep']=="customerName"){
                $field="customerName";
                }
                else if($_POST['dep']=="customer_phone"){
           ...
           $d=mysqli_query($con,"update transaction set $field='$value' where TransactionId=$transactionId");
       
              if($d){
               echo"<br/><p style='color:blue;font-size:20pt'> <i class='fa fa-check'></i> The Error has been corrected. The new ".$field." is ".$value."</p>";
